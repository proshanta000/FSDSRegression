...
class DataTransformation:
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            input_fuature_train_df = train_df.drop(columns=drop_column, axis=1)
            target_feature_train_df = train_df[target_column_name]

            input_feature_test_df = test_df.drop(columns=drop_column, axis=1)
            target_feature_test_df = test_df[target_column_name]

            logging.info('Applying preprocessing object on traning and test dataset.')

            ## Transformating using preprocessor object
            input_feature_train_arr = preprocessing_obj.fit_transform(input_fuature_train_df)
            ...
            logging.info('Excption ouccered in the initiate_data_transformation.')
            raise CustomException(e, sys)